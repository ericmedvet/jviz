...
public class HeatPolyMapPlotVideoBuilder extends AbstractXYPlotVideoBuilder<HeatPolyMapPlot, Map<Polygon, Double>> {

  public HeatPolyMapPlotVideoBuilder(
      Configuration c,
      io.github.ericmedvet.jviz.core.plot.image.Configuration iConfiguration
  ) {
    super(c, new HeatPolyMapPlotDrawer(iConfiguration));
  }

  @Override
  protected List<HeatPolyMapPlot> split(HeatPolyMapPlot p, SplitType splitType) {
    return split(p.dataGrid(), splitType).stream()
        .map(
            dg -> new HeatPolyMapPlot(
                p.title(),
                p.xTitleName(),
                p.yTitleName(),
                p.xName(),
