...
public abstract class AbstractXYPlotDrawer<P extends XYPlot<D>, D> implements XYPlotDrawer<P, D> {
  ...
  protected Grid<Axis> computeAxes(Graphics2D g, Layout l, boolean isXAxis, P p) {
    return computeRanges(isXAxis, isXAxis ? xExtensionRate : yExtensionRate, p).entries().stream()
        .map(e -> {
  ...
  protected Axis computeAxis(Graphics2D g, double size, D data, DoubleRange range, boolean isXAxis) {
    DoubleRange innerRange =
        PlotUtils.enlarge(range, configuration.general().plotDataRatio());
    double labelLineL = PlotUtils.computeStringH(g, configuration, Configuration.Text.Use.TICK_LABEL)
  ...
  protected DoubleRange plotRange(
    ...
    if (isXAxis
        && configuration.plotMatrix().independences().contains(Configuration.PlotMatrix.Independence.COLS)) {
      return colLargestRange;
    }
    if (!isXAxis
        && configuration.plotMatrix().independences().contains(Configuration.PlotMatrix.Independence.ROWS)) {
