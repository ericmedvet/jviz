...
public class VectorialFieldPlotDrawer extends AbstractXYPlotDrawer<VectorialFieldPlot, List<VectorialFieldDataSeries>> {
  ...
  protected DoubleRange computeRange(
      List<VectorialFieldDataSeries> data, boolean isXAxis, VectorialFieldPlot vectorialFieldPlot) {
    return data.stream()
  ...
  public double computeLegendH(Graphics2D g, VectorialFieldPlot p) {
    ...
    Point2D legendImageSize =
        new Point2D.Double(c.legendImageSizeRate() * gm.refL(), c.legendImageSizeRate() * gm.refL());
  ...
  public void drawPlot(Graphics2D g, GMetrics gm, Rectangle2D r, Key k, Axis xA, Axis yA, VectorialFieldPlot p) {
    ...
    Set<Entry<String, List<VectorialFieldDataSeries>>> dsEntries = p.dataGrid().values().stream()
        .map(TitledData::data)
        .flatMap(List::stream)
    ...
    Map<String, DoubleRange> dstYRanges = dsEntries.stream()
        .collect(Collectors.toMap(Entry::getKey, e -> e.getValue().stream()
            ...
            .reduce(DoubleRange::largest)
            .orElseThrow()));
    List<Point> srcPoints = p.dataGrid().values().stream()
        .map(TitledData::data)
        .flatMap(List::stream)
    ...
    p.dataGrid().get(k).data().forEach(ds -> {
      ...
      ds.pointPairs()
          .forEach((srcP, dstP) -> PlotUtils.drawArrow(
              g,
