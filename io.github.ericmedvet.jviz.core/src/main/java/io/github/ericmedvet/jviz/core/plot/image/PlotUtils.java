...
public class PlotUtils {
  ...
  public static <P extends XYPlot<D>, D> Layout computeLayout(
      ...
      Configuration c,
      P plot,
      XYPlotDrawer<P, D> plotDrawer
  ) {
    GMetrics gm = new GMetrics(g);
    double initialXAxisL = computeStringW(g, c, "0", Configuration.Text.Use.TICK_LABEL) + 2d * c.layout()
        .xAxisMarginHRate() * gm.h + c.layout().xAxisInnerMarginHRate() * gm.h;
    double initialYAxisL = computeStringW(g, c, "0", Configuration.Text.Use.TICK_LABEL) + 2d * c.layout()
    ...
    Layout l = new Layout(
        ...
        plot.dataGrid().w(),
        plot.dataGrid().h(),
        plot.title().isEmpty() ? 0 : (computeStringH(g, c, Configuration.Text.Use.TITLE) + 2d * c.layout()
            .mainTitleMarginHRate() * gm.h),
        plotDrawer.computeLegendH(g, plot) + 2d * c.layout().legendMarginHRate() * gm.h,
        c.plotMatrix().titlesShow().equals(Configuration.PlotMatrix.Show.BORDER) ? plot.dataGrid()
            .entries()
            .stream()
            .filter(e -> e.key().y() == 0)
            .map(e -> e.value().xTitle())
            .allMatch(String::isEmpty) ? 0 : (computeStringH(g, c, Configuration.Text.Use.AXIS_LABEL) + 2d * c.layout()
                .colTitleMarginHRate() * gm.h) : 0,
        c.plotMatrix().titlesShow().equals(Configuration.PlotMatrix.Show.BORDER) ? plot.dataGrid()
            .entries()
            .stream()
            .filter(e -> e.key().x() == plot.dataGrid().w() - 1)
        ...
        c.plotMatrix().titlesShow().equals(Configuration.PlotMatrix.Show.BORDER) ? 0 : plot.dataGrid()
            ...
            .map(e -> e.value().xTitle())
            .allMatch(String::isEmpty) ? 0 : (computeStringH(g, c, Configuration.Text.Use.AXIS_LABEL) + 2d * c.layout()
                .colTitleMarginHRate() * gm.h),
        c.plotMatrix().titlesShow().equals(Configuration.PlotMatrix.Show.BORDER) ? 0 : plot.dataGrid()
            .entries()
            .stream()
            .map(e -> e.value().yTitle())
            .allMatch(String::isEmpty) ? 0 : (computeStringH(g, c, Configuration.Text.Use.AXIS_LABEL) + 2d * c.layout()
                .rowTitleMarginWRate() * gm.w),
        plot.dataGrid()
            .keys()
            .stream()
    ...
    for (int i = 0; i < nOfIterations; i = i + 1) {
      ...
      List<String> xTickLabels = xAxesGrid.values()
          .stream()
          .map(Axis::labels)
          .flatMap(List::stream)
          .toList();
      List<String> yTickLabels = yAxesGrid.values()
          .stream()
          .map(Axis::labels)
          .flatMap(List::stream)
          .toList();
  ...
  public static void drawString(
    ...
    double w = switch (direction) {
      case H -> sW;
      case V -> sH;
    };
    double h = switch (direction) {
      case H -> sH;
      case V -> sW;
    };
  ...
  public static DoubleRange enlarge(DoubleRange range, double r) {
    return new DoubleRange(
        range.min() - range.extent() * (r - 1d) / 2d,
        range.max() + range.extent() * (r - 1d) / 2d
    );
  }
